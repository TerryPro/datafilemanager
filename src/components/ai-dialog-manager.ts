...
export class AiDialogManager {
  ...
  async openAiDialog(panel: NotebookPanel): Promise<void> {
    ...
    intentInput.placeholder =
      '例如：读取 dataset 下所有 CSV 合并为一个 DataFrame 并显示前 20 行';
    ...
    generateBtn.addEventListener('click', async () => {
      ...
      try {
        const payload = this.buildAiRequestPayload(
          panel,
          source,
          intentInput.value,
          modeSelect.value,
          ...
          if (aiResp.suggestion) {
            try {
              await this.applySuggestion(
                panel,
                aiResp.suggestion,
                modeSelect.value as 'replace' | 'insert' | 'append' | 'explain',
                autoRun.checked
              );
              // 关闭对话框
              dialog.remove();
  ...
  private buildAiRequestPayload(
      ...
      if (cell && cell.model.type === 'code') {
        ...
        for (let i = 0; i < outputs.length; i++) {
          ...
          if (out.type === 'stream' && (out.data as any).name === 'stderr') {
            ...
            output += `[error] ${(out.data as any).ename}: ${
              (out.data as any).evalue
            }\n`;
            if ((out.data as any).traceback) {
              output += `Traceback:\n${(
                (out.data as any).traceback as string[]
              ).join('\n')}\n`;
            }
          } else if (
    ...
    return {
      ...
      context: ctx,
      intent,
      options: { mode, autoRun, privacy: 'normal' },
      output
    };
  }
  ...
  private async applySuggestion(
    panel: NotebookPanel,
    suggestion: string,
    mode: 'replace' | 'insert' | 'append' | 'explain' | 'fix',
    autoRun: boolean
  ): Promise<void> {
    const content = panel.content;
