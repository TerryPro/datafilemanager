...
export const generateCode = (nodes: Node[], edges: Edge[]): string => {
  ...
  sortedNodes.forEach(node => {
    ...
    const schema: NodeSchema | undefined = data.schema;

    const outputVarName = `df_${nodeId.replace(/-/g, '_')}`;

    if (type === 'csv_loader') {
      // Legacy / Specific Node
      outputVars[nodeId] = outputVarName;
      const filepath = data.filepath || 'dataset/data.csv';
      lines.push(`# Load CSV (Legacy Node)`);
      lines.push(`${outputVarName} = pd.read_csv('${filepath}')`);
      lines.push(`print(f"Loaded ${outputVarName} with shape: {${outputVarName}.shape}")`);
      ...
      if (inputEdge) {
        ...
        const inputVar = outputVars[sourceId];
        if (inputVar) {
            const plotType = data.plotType || 'line';
            const column = data.column;
            const yParam = column ? `, y='${column}'` : '';
            lines.push(`# Plot (Legacy Node)`);
            lines.push(`${inputVar}.plot(kind='${plotType}'${yParam})`);
            lines.push(`plt.show()`);
        }
      }

    } else if (type === 'generic' && schema) {
      ...
      schema.args?.forEach(arg => {
          ...
          if (val === undefined) val = arg.default;
