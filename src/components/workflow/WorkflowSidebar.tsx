...
export const WorkflowSidebar = () => {
  const [library, setLibrary] = useState<Record<string, NodeSchema[]>>({});
  ...
  const onDragStart = (event: React.DragEvent, schema: NodeSchema) => {
  ...
  const filteredLibrary = Object.entries(library).reduce((acc, [category, nodes]) => {
  ...
  return (
    <aside style={{ 
      ...
      <div style={{ 
        ...
        <div className="jp-InputGroup" style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
          <input
            type="text"
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading ? (
          <div style={{ padding: '10px', color: 'var(--jp-ui-font-color2)' }}>Loading library...</div>
        ) : (
              ...
              >
                <span style={{ marginRight: '8px', display: 'flex', alignItems: 'center' }}>
                  {expandedCategories.has(category) || searchTerm ? (
                    <caretDownIcon.react tag="span" width="16px" height="16px" />
                  ) : (
                ...
                <div style={{ backgroundColor: 'var(--jp-layout-color1)' }}>
                  {nodes.map(node => (
                    <div 
                      ...
                      onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--jp-layout-color2)'}
