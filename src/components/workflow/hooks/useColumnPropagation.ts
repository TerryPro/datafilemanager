...
const topologicalSort = (nodes: Node[], edges: Edge[]): Node[] => {
  ...
  const visit = (nodeId: string) => {
    if (visiting.has(nodeId)) return; // Cycle detected or processing
...
export const useColumnPropagation = (
  ...
  const propagate = useCallback(async () => {
    ...
    for (const node of sortedNodes) {
      ...
      try {
        if (schema.category === 'source' || schema.id === 'load_csv' || schema.id === 'import_variable') {
           // Source Node Logic
           ...
           outputColumns = transformer(inputColumns, node.data.values || {}, schema);
        }
      } catch (e) {
