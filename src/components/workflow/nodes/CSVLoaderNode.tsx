...
export const CSVLoaderNode = memo(({ data, selected }: { data: CSVLoaderNodeData; selected?: boolean }) => {
  const [files, setFiles] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchFiles = async () => {
      if (!data.serviceManager) return;
  ...
  const handleChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    data.filepath = newValue;
    // Notify parent to update columns
    if (data.onFileChange) {
      data.onFileChange(data.id, newValue);
    }
  };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ padding: '12px' }}>
        <label style={{ display: 'block', marginBottom: '2px', color: 'var(--jp-ui-font-color2)', fontSize: '10px' }}>File:</label>
        {loading ? (
