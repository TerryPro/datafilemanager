...
export const PlotNode = memo(({ data, selected }: { data: PlotNodeData; selected?: boolean }) => {
  const inputStyle = {
    width: '100%',
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ position: 'relative', width: '1px', height: '1px', display: 'flex', justifyContent: 'center' }}>
          <Handle
            type="target"
      ...
      <div style={{ padding: '12px' }}>
        <label style={{ display: 'block', marginBottom: '2px', color: 'var(--jp-ui-font-color2)', fontSize: '10px' }}>Type:</label>
        <select 
          className="nodrag" 
          defaultValue={data.plotType || 'line'}
          onChange={(evt) => data.plotType = evt.target.value}
          style={inputStyle}
        >
        ...
        <label style={{ display: 'block', marginBottom: '2px', color: 'var(--jp-ui-font-color2)', fontSize: '10px' }}>Column:</label>
        {data.columns && data.columns.length > 0 ? (
          <select 
            className="nodrag" 
            defaultValue={data.column || ''} 
            onChange={(evt) => data.column = evt.target.value}
