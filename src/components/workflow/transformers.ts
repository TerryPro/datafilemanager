...
const getDefaultInputCols = (
  ...
  if (inputColumns['df_in']) {
    return inputColumns['df_in'];
  }
  return inputColumns[keys[0]];
};

// 1. Pass-through Transformer
export const transformPassThrough: TransformerFunc = (
  inputs,
  params,
  schema
) => {
...
export const transformSelectColumns: TransformerFunc = (
  inputs,
  params,
  schema
) => {
...
export const transformRenameColumns: TransformerFunc = (
  inputs,
  params,
  schema
) => {
...
export const transformMerge: TransformerFunc = (inputs, params, schema) => {
  ...
  const onCols: string[] = Array.isArray(onParam)
    ? onParam
    : onParam
    ? [onParam]
    : [];
  ...
  onCols.forEach(name => {
    ...
    const original =
      leftCols.find(c => c.name === name) ||
      rightCols.find(c => c.name === name);
    if (original) {
  ...
  leftCols.forEach(col => {
    ...
    const conflict = rightCols.some(
      rc => rc.name === col.name && !onCols.includes(rc.name)
    );
  ...
  rightCols.forEach(col => {
    ...
    const conflict = leftCols.some(
      lc => lc.name === col.name && !onCols.includes(lc.name)
    );
