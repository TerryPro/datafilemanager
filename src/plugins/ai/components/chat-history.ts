...
export class ChatHistory extends Widget {
  ...
  constructor(props: ChatHistoryProps) {
    ...
    if (props.messages && props.messages.length > 0) {
      try {
        ...
        console.error('[ChatHistory] Failed to render initial messages:', error);
  ...
  addMessage(message: ChatMessage): void {
    try {
      ...
      if (!message || !message.id || !message.sender || !message.content) {
        console.warn('[ChatHistory] Invalid message format:', message);
        return;
      }
      
      // Render the message using MessageItem component
  ...
  clear(): void {
    try {
      ...
      try {
        ...
        console.error('[ChatHistory] Failed to clear using innerHTML:', innerError);
      }
    }
  }
  ...
  private showError(errorText: string): void {
    try {
      ...
      errorElement.className = 'ai-sidebar-message ai-sidebar-message-system error';
