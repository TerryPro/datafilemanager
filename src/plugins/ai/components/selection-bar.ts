...
export class SelectionBar extends Widget {
  private props: ISelectionBarProps;
   ...
   */
  constructor(props: ISelectionBarProps) {
    super();
    this.props = props;
  ...
  private createVariableChip(): HTMLDivElement {
    ...
    // Clear button
    const clearBtn = createButton('ai-chip-clear', '×', '清除已选变量', () =>
  ...
  private createAlgorithmChip(): HTMLDivElement {
    ...
    // Clear button
    const clearBtn = createButton('ai-chip-clear', '×', '清除已选算法', () =>
   ...
   * ```
   */
  updateProps(props: Partial<ISelectionBarProps>): void {
