 ...
 * @returns Array of hunk descriptors
 */
export function groupIntoHunks(ops: IDiffOp[]): IDiffHunk[] {
  ...
  for (let i = 0; i < ops.length; i++) {
    const op = ops[i];
    if (op.type !== 'ctx') {
      // If we're not in a hunk, start one
      if (currentStart === -1) {
        currentStart = i;
      }
    } else {
      // If we were in a hunk, end it
      if (currentStart !== -1) {
        hunks.push({ start: currentStart, end: i - 1 });
        currentStart = -1;
      }
    }
  }

  // Close the last hunk if it extends to the end
  if (currentStart !== -1) {
    hunks.push({ start: currentStart, end: ops.length - 1 });
  }
 ...
 * ```
 */
export function buildTextFromDecisions(
  ops: IDiffOp[],
  decisions: Decision[]
): string {
  const result: string[] = [];
