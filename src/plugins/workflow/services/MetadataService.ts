...
export class MetadataService {
  ...
  async fetchCSVColumns(filepath: string): Promise<IColumn[]> {
    ...
    try {
      ...
      return (data.columns || []).map((name: string) => ({
        name,
        type: 'unknown'
      }));
    } catch (err) {
  ...
  async fetchVariableColumns(variableName: string): Promise<IColumn[]> {
    if (!this.serviceManager || !variableName) {
      return [];
    }
    ...
    try {
      ...
      const sessionConnection = this.serviceManager.sessions.connectTo({
        model: sessionModel
      });
        ...
        future.onIOPub = (msg: any) => {
          if (
            ...
            try {
              ...
              console.error(
                'MetadataService: Failed to parse variable columns',
                e
              );
            }
          }
